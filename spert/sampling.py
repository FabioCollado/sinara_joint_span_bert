...
def create_train_sample(doc, neg_entity_count: int, neg_rel_count: int, max_span_size: int, rel_type_count: int):
    ...
    # print(doc.encoding) #['[CLS]', 'A', 'Constituição', 'Federal', 'de', '1988', 'estabelece', ...])
    for e in doc.entities:
        pos_entity_spans.append(e.span) # (2, 6)
        pos_entity_types.append(e.entity_type.index)
        pos_entity_masks.append(create_entity_mask(*e.span, context_size))
        #tensor([False, False,  True,  True,  True,  True, False, False, False, False, False, ...])
        pos_entity_sizes.append(len(e.tokens)) #4
    ...
    for pair, rels in entity_pair_relations.items():
        ...
        # Lembre-se que input_reader.relation_types['P'].index = 2 (índice do 'None' é 1 e não tem 0)
